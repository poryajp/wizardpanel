...
function getMarzneshinToken($server_id)
{
    ...
    if ($cached_token = $stmt_cache->fetchColumn())
        return $cached_token;
    ...
    $server_info = $stmt->fetch();
    if (!$server_info)
        return ['error' => "Server info not found for server ID: {$server_id}"];
...
function resetMarzneshinUserUsage($username, $server_id)
{
    // مرزنشین همانند مرزبان از endpoint reset پشتیبانی می‌کند
    $response = marzneshinApiRequest("/api/users/{$username}/reset", $server_id, 'POST');
    return $response && isset($response['username']);
}