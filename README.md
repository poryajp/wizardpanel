...
## 📝 تغییرات و اصلاحات اخیر (Recent Updates \u0026 Fixes)

این نسخه شامل اصلاحات زیر جهت بهبود عملکرد و رفع ایرادات نسخه اصلی می‌باشد:

### 1️⃣ اصلاح فرمت نام کاربری (Username Generation)
- **فایل:** `src/includes/functions.php`
- **تغییر:** الگوی تولید نام کاربری برای سرویس‌های جدید تغییر کرد.
- **فرمت جدید:** `ChatID` + `_` + `2 Random Digits`
- **مثال:** `123456789_45`

### 2️⃣ اصلاح نمایش دکمه‌های خرید (RTL Fix)
- **فایل‌ها:** `src/includes/functions.php` و `src/bot.php`
- **تغییر:** جایگاه «قیمت» و «حجم» در دکمه‌های شیشه‌ای جابجا شد.
- **هدف:** حل مشکل بهم‌ریختگی متن فارسی در تلگرام (قرارگیری صحیح کلمه "تومان").
- **فرمت نمایش:** `20,000 تومان | 10 GB`

### 3️⃣ رفع خطای مسیردهی کرون‌جاب (Cron Job Path Fix)
- **فایل:** `src/cron.php`
- **تغییر:** اصلاح مسیر فراخوانی (Require) فایل‌های API.
- **توضیح:** مسیر فایل‌ها از پوشه اشتباه `includes/` به پوشه صحیح `api/` تغییر یافت تا از خطای `PHP Fatal error: Failed opening required` جلوگیری شود.
